...
class TimeFilter(ModelBase):
    def __init__(self, **kwargs):
        super(TimeFilter, self).__init__()
        self.config = TimeFilterConfig(**kwargs)
    ...
    def validate(
        ...
        for input, target, input_mark, target_mark in valid_data_loader:
            ...
            output, _ = self.model(input, self.masks, is_training=False)
    ...
    def _get_mask(self):
        ...
        masks = []
        for k in range(L):
            S = ((torch.arange(L) % N == k % N) & (torch.arange(L) != k)).to(dtype).cuda()
            T = ((torch.arange(L) >= k // N * N) & (torch.arange(L) < k // N * N + N) & (torch.arange(L) != k)).to(dtype).cuda()
            ST = torch.ones(L).to(dtype).cuda() - S - T
            ST[k] = 0.0
        ...
        **kwargs,
        ...
        for epoch in range(config.num_epochs):
            ...
            for i, (input, target, input_mark, target_mark) in enumerate(
                ...
                output, moe_loss = self.model(input, self.masks, is_training=True)
                alpha = 0.05
                target = target[:, -config.horizon :, :series_dim]
                output = output[:, -config.horizon :, :series_dim]
                loss = criterion(output, target) + alpha * moe_loss
