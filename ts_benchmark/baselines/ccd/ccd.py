...
from ts_benchmark.baselines.ccd.models.CCD_Model import Model
    ...
    def validate(
        ...
        with torch.no_grad():
            for input, target, input_mark, target_mark in valid_data_loader:
                ...
                input = input[:,-config.seq_len:, :series_dim]
        ...
        for epoch in range(config.num_epochs):
            ...
            for i, (input, target, input_mark, target_mark) in enumerate(
                ...
                if self.config.lradj == "TST":
                    adjust_learning_rate(optimizer, epoch + 1, config, scheduler=scheduler, print_out=False)
                    scheduler.step()
