...
class TransformerAdapter(ModelBase):
    ...
    def validate(
        ...
        inference_time_ms = time.time()-start
    ...
    def forecast_fit(
        ...
        **kwargs,
        ...
        for epoch in range(config.num_epochs):
            ...
            inference_time_ms = time.time()-start
            if self.epoch_now > 2:
                self.train_time.append(inference_time_ms)
            if train_ratio_in_tv != 1:
            ...
            if len(self.valid_time)>=2:
                print(f'valid_time:{sum(self.valid_time)/len(self.valid_time)}')       
                print(f'train_time:{sum(self.train_time)/len(self.train_time)}')      
