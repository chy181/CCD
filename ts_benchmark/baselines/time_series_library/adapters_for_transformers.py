...
class TransformerAdapter(ModelBase):
    ...
    def validate(
        ...
        with torch.profiler.profile(
                ...
                on_trace_ready=torch.profiler.tensorboard_trace_handler('./log/maptr_shiyu'),
                record_shapes=True,
                ...
                with_stack=True
                    ...
                    for input, target, input_mark, target_mark in valid_data_loader:
                        ...
                        with torch.profiler.record_function("model::validate"):
                        ...
                        prof.step()
                    event_list = prof.key_averages(group_by_stack_n=5)
                    filtered_events_aten = [e for e in event_list if "model::" in e.key]
                    for event in sorted(filtered_events_aten, key=lambda x: x.cpu_time_total, reverse=True):
                        print(f"{event.key} {event.cpu_time_total/1000:4} {event.cuda_time/1000:4}")
            ...
            with torch.profiler.profile(
                ...
                for i, (input, target, input_mark, target_mark) in enumerate(
                    ...
                    if epoch==0 and i==0:
                        ...
                        import time                    
                        macs, params = profile(self.model,(input, input_mark, dec_input, target_mark))
                        print(f'params: {params/1000000}, MACs: {macs/1024/1024/1024}')
                    with torch.profiler.record_function("model::train"):
                        output = self.model(input, input_mark, dec_input, target_mark)
                        target = target[:, -config.horizon :, :series_dim]
                        output = output[:, -config.horizon :, :series_dim]
