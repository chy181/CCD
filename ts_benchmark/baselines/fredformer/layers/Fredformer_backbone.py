...
class Fredformer_backbone(nn.Module):
    def __init__(self, ablation:int,  mlp_drop:float, use_nys:int, output:int, mlp_hidden:int,cf_dim:int,cf_depth :int,cf_heads:int,cf_mlp:int,cf_head_dim:int,cf_drop:float,c_in:int, context_window:int, target_window:int, patch_len:int, stride:int,  d_model:int, 
        ...
        self.head_f3 = Flatten_Head(self.individual, self.n_vars, d_model * 2 *(int((target_window - patch_len)/stride + 1)), target_window, head_dropout=head_dropout)
        self.head_f4 = Flatten_Head(self.individual, self.n_vars, d_model * 2 *(int((target_window - patch_len)/stride + 1)), target_window, head_dropout=head_dropout)
    ...
    def forward(self, z):                                                                   # z: [bs x nvars x seq_len]
        ...
        z = torch.fft.ifft(torch.log(frequency + 1e-8))
