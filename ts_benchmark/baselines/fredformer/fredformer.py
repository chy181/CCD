...
from ts_benchmark.baselines.fredformer.models.Fredformer_Model import Model
...
class MutiDomainLoss(nn.Module):
    def __init(self):
        super(MutiDomainLoss, self).__init__()
        self.mse = nn.MSELoss()
    def forward(self, output, target, denorm, domain='time,frequency,quefrency'):
        
        output_time, output_frequency, output_quefrency = output
        target_time = target
        # target = denorm(target ,'norm_no_update').permute(0,2,1)
        target = denorm(target ,'norm').permute(0,2,1)
        target_frequency = torch.fft.fft(target)
        target_quefrency = torch.fft.ifft(torch.log(target_frequency+1e-8))
        loss=0
        if 'time' in domain:
            time = ((output_time - target_time).abs() ** 2).mean()
            loss+=time
        if 'frequency' in domain:
            frequency = ((output_frequency - target_frequency).abs() ** 2).mean()
            loss+=frequency*0.01
            
        if 'quefrency' in domain:
            quefrency = ((output_quefrency - target_quefrency).abs() ** 2).mean()
            loss+=quefrency*0.01
    ...
    def validate(
        ...
        with torch.no_grad():
            for input, target, input_mark, target_mark in valid_data_loader:
                ...
                output,output_frequency,output_quefrency = self.model(input)
                target = target[:, -config.horizon :, :series_dim]
                output = output[:, -config.horizon :, :series_dim]
            
                # loss = criterion((output,output_frequency,output_quefrency), target,  self.model.model.revin_layer, domain='time')
                loss = criterion((output,output_frequency,output_quefrency), target,  self.model.model.revin_layer, domain='time')
        ...
        **kwargs,
        ...
        for epoch in range(config.num_epochs):
            ...
            for i, (input, target, input_mark, target_mark) in enumerate(
                ...
                # total_loss = criterion((output,output_frequency,output_quefrency), target,  self.model.model.revin_layer,domain='time,frequency,quefrency')
                total_loss = criterion((output,output_frequency,output_quefrency), target,  self.model.model.revin_layer,domain='time')
            ...
            adjust_learning_rate(optimizer, epoch + 1, config, scheduler=scheduler)
